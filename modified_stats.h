...
#define FIXED_SLICE_VALUE 750000UL  // 0.75ms를 ns 단위로 표현
...
static inline void sched_info_depart(struct rq *rq, struct task_struct *t)
{
	...
	// user
	if ((strcmp(t->comm, "cpu") == 0) || (strcmp(t->comm, "cpuChild") == 0))
	{	
		pid_t pid = t->pid;

		int nice_val = task_nice(t);

		u64 vruntime = t->se.vruntime;

		u64 vdeadline = vruntime + (FIXED_SLICE_VALUE / (nice_val+21));

		unsigned long cpu_burst_time = delta;
		
		printk_deferred(KERN_INFO "SCHED_LOG: pid=%d, nice=%d, vruntime=%llu, deadline=%llu, burst_ns=%lu comm=%s\n",
		pid, nice_val, vruntime, vdeadline, cpu_burst_time, t->comm);
	}
}
